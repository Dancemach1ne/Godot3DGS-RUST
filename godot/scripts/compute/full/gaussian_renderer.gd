...
var _did_report_projection_state := false
...
func render_for_compositor(
	...
	if state.context == null or state.gpu_build_failed:
		return {}
...
func _rasterize_state(state, point_count: int, light_count: int) -> void:
	...
	_report_projection_state_once(state)
...
func _report_projection_state_once(state) -> void:
	if _did_report_projection_state or state.context == null:
		return
	_did_report_projection_state = true
	var histogram: PackedByteArray = state.context.device.buffer_get_data(state.descriptors["histogram"].rid)
	var transforms: PackedByteArray = state.context.device.buffer_get_data(state.descriptors["instance_transforms"].rid)
	var splats: PackedByteArray = state.context.device.buffer_get_data(state.descriptors["splats"].rid)
	var generated_tiles := histogram.decode_u32(0) if histogram.size() >= 4 else -1
	var visible_flag := transforms.decode_float(12) if transforms.size() >= 16 else -1.0
	var splat_time := splats.decode_float(12) if splats.size() >= 16 else -999.0
	print("[Godot3DGS-RUST] Projection diagnostic: tiles=%d, instance_visible=%s, splat_time=%s" % [generated_tiles, visible_flag, splat_time])
