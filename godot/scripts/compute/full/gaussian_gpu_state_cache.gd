...
const RenderingDeviceContext := preload("res://scripts/compute/full/gaussian_rendering_device_context.gd")
...
const SHADER_PATH_PROJECTION := "res://scripts/compute/full/shaders/gsplat_projection.glsl"
const SHADER_PATH_RADIX_UPSWEEP := "res://scripts/compute/full/shaders/radix_sort_upsweep.glsl"
const SHADER_PATH_RADIX_SPINE := "res://scripts/compute/full/shaders/radix_sort_spine.glsl"
const SHADER_PATH_RADIX_DOWNSWEEP := "res://scripts/compute/full/shaders/radix_sort_downsweep.glsl"
const SHADER_PATH_BOUNDARIES := "res://scripts/compute/full/shaders/gsplat_boundaries.glsl"
const SHADER_PATH_RENDER := "res://scripts/compute/full/shaders/gsplat_render.glsl"
...
func get_or_create_render_state(texture_size: Vector2i):
	...
	if state == null:
		...
		state.tile_dims = Vector2i(
			floori(float(texture_size.x + TILE_SIZE - 1) / float(TILE_SIZE)),
			floori(float(texture_size.y + TILE_SIZE - 1) / float(TILE_SIZE))
		)
...
func rebuild_gpu_state(state, point_count: int, unique_data_size: int, instance_count: int) -> void:
	...
	for shader_value in state.shaders.values():
		var shader_rid: RID = shader_value
		if not shader_rid.is_valid():
			# Do not retry every frame: a failed import would otherwise attempt to
			# free a RenderingDevice context while the compositor has it locked.
			state.gpu_build_failed = true
			state.needs_gpu_rebuild = false
			push_error("[Godot3DGS-RUST] Full Compute pipeline disabled until its GLSL shaders import successfully.")
			return

	var num_sort_elements_max := point_count * MAX_SORT_ELEMENTS_PER_SPLAT
	var num_partitions := floori(float(num_sort_elements_max + PARTITION_SIZE - 1) / float(PARTITION_SIZE))
	...
	var unique_splat_count := maxi(1, floori(float(unique_data_size) / float(FLOATS_PER_SPLAT * BYTES_PER_FLOAT)))
