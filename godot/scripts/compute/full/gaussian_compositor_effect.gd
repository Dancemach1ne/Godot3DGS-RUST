...
const MANAGER_SCRIPT := preload("res://scripts/compute/full/gaussian_render_manager.gd")
const DIRECT_TEXTURE_SHADER := preload("res://scripts/compute/full/shaders/direct_texture_overlay.gdshader")
...
@export_enum("Compositor", "Direct Texture") var display_mode: int:
	set(value):
		_display_mode = clampi(value, DisplayMode.COMPOSITOR, DisplayMode.DIRECT_TEXTURE) as DisplayMode
...
var _display_mode: DisplayMode = DisplayMode.COMPOSITOR
...
var _diagnostic_stage := ""
var _did_report_output_bytes := false
...
func _render_callback(_effect_callback_type: int, render_data: RenderData) -> void:
	...
	if not is_direct_texture_mode and (not rd or not shader.is_valid() or not pipeline.is_valid()):
		_diagnose("compositor shader is unavailable")
		_queue_direct_texture_overlay_state(false, RID())
		return
	...
	if scene_buffers == null or scene_data == null:
		_diagnose("RenderData has no RD scene buffers/data")
		_queue_direct_texture_overlay_state(false, RID())
		return

	var manager = MANAGER_SCRIPT.get_instance()
	if manager == null:
		_diagnose("GaussianRenderManager instance is unavailable")
		_queue_direct_texture_overlay_state(false, RID())
		return

	var size: Vector2i = scene_buffers.get_internal_size()
	if size.x <= 0 or size.y <= 0:
		_diagnose("render target has no valid size")
		_queue_direct_texture_overlay_state(false, RID())
		return
	...
	for view in scene_buffers.get_view_count():
		...
		if gsplat_result.is_empty():
			_diagnose("renderer returned no GPU output")
			continue
		...
		if not gsplat_texture.is_valid() or not gsplat_depth_texture.is_valid():
			_diagnose("renderer returned invalid output textures")
			continue
		_diagnose("renderer returned valid GPU textures")
		_report_output_texture_once(gsplat_texture)
...
func _diagnose(stage: String) -> void:
	if _diagnostic_stage == stage:
		return
	_diagnostic_stage = stage
	print("[Godot3DGS-RUST] Compute diagnostic: %s" % stage)

func _report_output_texture_once(texture: RID) -> void:
	if _did_report_output_bytes or rd == null:
		return
	_did_report_output_bytes = true
	var bytes := rd.texture_get_data(texture, 0)
	var sampled_nonzero := 0
	var stride := maxi(1, bytes.size() / 2048)
	for index in range(0, bytes.size(), stride):
		if bytes[index] != 0:
			sampled_nonzero += 1
	print("[Godot3DGS-RUST] Compute output diagnostic: %d bytes, %d non-zero sampled bytes" % [bytes.size(), sampled_nonzero])
...
func initialize_compute_shader() -> void:
	...
	var glsl_file: RDShaderFile = load("res://scripts/compute/full/shaders/gaussian_composite.glsl")
	if glsl_file == null:
		return
